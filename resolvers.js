...
const resolvers = {
    Query: {
        ...
        countryByName: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountryByName(args.name)
            return countryData.then(data => data[0])
        },

        countryByFullName: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountryByFullName(args.name)
            return countryData.then(data => data[0])
        },

        countryByISOCode: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountryByISOCode(args.code)
            return countryData.then(data => data[0])
        },

        countriesByISOCodes: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountriesByISOCodes(args.codes)
            return countryData
        },

        countryByCurrency: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountryByCurrency(args.currency)
            return countryData.then(data => data[0])
        },

        countriesByLanguage: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountriesByLanguage(args.language)
            return countryData
        },

        countryByCapitalCity: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountryByCapitalCity(args.city)
            return countryData.then(data => data[0])
        },

        countryByCallingCode: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountryByCallingCode(args.code)
            return countryData.then(data => data[0])
        },

        countriesByRegion: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountriesByRegion(args.region)
            return countryData
        },
        ...
        countriesBySubregion: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getAllCountries()
        ...
        countriesByRegionalBloc: async (parent, args, ctx) => {
            let countryData = ctx.dataSources.countriesAPI.getCountriesByRegionalBloc(args.bloc)
            return countryData
        },
    },
}
